...
static void parse_command_line_arguments_init_default_result(
	...
	if (parse_options->default_result->arguments != NULL) {
		for (size_t i = 0; parse_options->default_result->arguments[i] != NULL;
			 i++) {
			(*results)->arguments =
				realloc((*results)->arguments, (i + 1) * sizeof(char*));
			(*results)->arguments[i] =
				parse_options->default_result->arguments[i];
			(*results)->nb_args++;
		}
	}
	...
	if (parse_options->default_result->options_values != NULL) {
		for (size_t i = 0; i < nb_options; i++) {
			if (!options[i].takes_arg
				&& parse_options->default_result->options_values[i] == NULL)
				continue;
			(*results)->options_values[i] =
				parse_options->default_result->options_values[i];
		}
	}
}
...
static void parse_command_line_arguments_init(
	...
	*nb_options = 0;
	while (options[*nb_options].shorts != NULL
		   || options[*nb_options].longs != NULL)
		(*nb_options)++;
	...
	*results = calloc(1, sizeof(struct Results));
	...
	if (parse_options->default_help != 0) {
		...
		(*results)->options_values = realloc((*results)->options_values,
											 (*nb_options + 1) * sizeof(char*));
...
static int parse_command_line_arguments_short_option_case(
	...
	parse_command_line_arguments_short_option_for_loop(
		options, i, argv, results, *multiple_short_options_indicies, nb_options,
		e);
...
parse_command_line_arguments_help(struct ParseOptions* parse_options,
	...
	if (parse_options->default_help) {
		...
		if (parse_options->default_help_value && results->nb_args == 0
			&& results->options_found == 0)
...
parse_command_line_arguments(char** argv, struct Option* options,
	...
	parse_command_line_arguments_help(parse_options, options, nb_options,
									  results);
