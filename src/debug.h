...
// | it only allows for one or zero %s, it corresponds to the local variable
// name | example: for (int i = 0; i < 3; i++) { DBGI(i); } will print: |
// (DBG):i = 0 | (DBG):i = 1 | (DBG):i = 2 | and yes doing DBGI(5) will print
// (DBG):5 = 5 as stupid as it looks
// v
#define DBG_VAR_NAME
...
dbgdec_DBGI(int) dbgdec_DBGI(size_t) dbgdec_DBGI(float) dbgdec_DBGI(double)
		dbgdec_DBGI(char) dbgdec_DBGI(long) dbgdec(void, DBGI_long_long,
																							 long long n)
...
#define DBGI(n)                                                                \
	_Generic((n), int                                                            \
					 : DBGI_int, const int                                               \
					 : DBGI_int, size_t                                                  \
					 : DBGI_size_t, const size_t                                         \
					 : DBGI_size_t, float                                                \
					 : DBGI_float, const float                                           \
					 : DBGI_float, double                                                \
					 : DBGI_double, const double                                         \
					 : DBGI_double, char                                                 \
					 : DBGI_char, const char                                             \
					 : DBGI_char, long                                                   \
					 : DBGI_long, const long                                             \
					 : DBGI_long, long long                                              \
					 : DBGI_long_long, const long long                                   \
					 : DBGI_long_long, unsigned char                                     \
					 : DBGI_unsigned_char, const unsigned char                           \
					 : DBGI_unsigned_char, default                                       \
					 : DBGI_int)(dbgmac_args(n, n))

/* DBGBIN */

#define dbgdec_DBGBIN(type) dbgdec(void, CAT(DBGBIN_, type), type n)

						dbgdec_DBGBIN(int) dbgdec_DBGBIN(size_t) dbgdec_DBGBIN(char)
								dbgdec_DBGBIN(long) dbgdec(void, DBGBIN_long_long, long long n)
										dbgdec(void, DBGBIN_unsigned_char, unsigned char n)

#define DBGBIN(n)                                                              \
	_Generic((n), int                                                            \
					 : DBGBIN_int, const int                                             \
					 : DBGBIN_int, size_t                                                \
					 : DBGBIN_size_t, const size_t                                       \
					 : DBGBIN_size_t, char                                               \
					 : DBGBIN_char, const char                                           \
					 : DBGBIN_char, long                                                 \
					 : DBGBIN_long, const long                                           \
					 : DBGBIN_long, long long                                            \
					 : DBGBIN_long_long, const long long                                 \
					 : DBGBIN_long_long, unsigned char                                   \
					 : DBGBIN_unsigned_char, const unsigned char                         \
					 : DBGBIN_unsigned_char, default                                     \
					 : DBGBIN_int)(dbgmac_args(n, n))

/* DBGA */

#define dbgdec_DBGA(type)                                                      \
	dbgdec(void, CAT(DBGA_, type), const type *array, size_t len)

												dbgdec_DBGA(int) dbgdec_DBGA(size_t) dbgdec_DBGA(float)
														dbgdec_DBGA(double) dbgdec_DBGA(char)
																dbgdec(void, DBGA_str, const char **array,
																			 size_t len)

#define DBGA(array, length_array)                                              \
	_Generic((array), \
	int*: DBGA_int, \
	const int*: DBGA_int, \
	size_t*: DBGA_size_t, \
...
#define DBGMAT_entry_width(entry)                                              \
	_Generic((entry), \
...
void DBGMAT_entries_width_int(size_t n, size_t m, const int **mat,
															int max_width[m]);
void DBGMAT_entries_width_size_t(size_t n, size_t m, const size_t **mat,
																 int max_width[m]);
void DBGMAT_entries_width_float(size_t n, size_t m, const float **mat,
																int max_width[m]);
void DBGMAT_entries_width_double(size_t n, size_t m, const double **mat,
																 int max_width[m]);
void DBGMAT_entries_width_char(size_t n, size_t m, const char **mat,
															 int max_width[m]);
void DBGMAT_entries_width_str(size_t n, size_t m, const char ***mat,
															int max_width[m]);

void DBGAMAT_entries_width_int(size_t n, size_t m, const int mat[n][m],
															 int max_width[m]);
void DBGAMAT_entries_width_size_t(size_t n, size_t m, const size_t mat[n][m],
																	int max_width[m]);
void DBGAMAT_entries_width_float(size_t n, size_t m, const float mat[n][m],
																 int max_width[m]);
void DBGAMAT_entries_width_double(size_t n, size_t m, const double mat[n][m],
																	int max_width[m]);
void DBGAMAT_entries_width_char(size_t n, size_t m, const char mat[n][m],
																int max_width[m]);
void DBGAMAT_entries_width_str(size_t n, size_t m, const char *mat[n][m],
															 int max_width[m]);

#define DBGMAT_entries_width(mat, n, m, max_width)                             \
	_Generic((mat), \
...
#define dbgdec_DBGMAT(type)                                                    \
	dbgdec(void, CAT(DBGMAT_, type), size_t n, size_t m, const type **mat)

dbgdec_DBGMAT(int) dbgdec_DBGMAT(size_t) dbgdec_DBGMAT(float)
		dbgdec_DBGMAT(double) dbgdec_DBGMAT(char)
				dbgdec(void, DBGMAT_str, size_t n, size_t m, const char ***mat)

#define dbgdec_DBGAMAT(type)                                                   \
	dbgdec(void, CAT(DBGAMAT_, type), size_t n, size_t m, const type mat[n][m])

						dbgdec_DBGAMAT(int) dbgdec_DBGAMAT(size_t) dbgdec_DBGAMAT(float)
								dbgdec_DBGAMAT(double) dbgdec_DBGAMAT(char)
										dbgdec(void, DBGAMAT_str, size_t n, size_t m,
													 const char *mat[n][m])

#define DBGMAT(width, height, mat)                                             \
	_Generic((mat), \
	int**: DBGMAT_int, \
